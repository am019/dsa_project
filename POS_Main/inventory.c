...
/* Helper to find minimum value node (used for 2-child case) */
...
/* Public function to delete an item */
void deleteItem() {
    if (!root) {
        printf("No inventory present. Load or add items first.\n");
        return;
    }
    ...
    printf("✅ Item ID %d deleted from BST.\n", id);

    // Persist updated inventory to file
    saveInventory();
    printf("💾 Inventory file updated successfully.\n");
}
